...
const initialColumns = {
  Doing: {
    id: 'Doing',
    title: 'Doing',
    tasks: [{ id: '1', title: 'Startup Ipsilon Spring', date: '4 Jun' }],
  },
  'To Do': {
    id: 'To Do',
    title: 'To Do',
  ...
  Ideas: {
    id: 'Ideas',
    title: 'Ideas',
    tasks: [{ id: '9', title: 'New e-commerce for desi', date: '27 Jun' }],
  },
};
function Board() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:3001/api/boards/${id}`)
      .then((response) => {
  ...
  const onDragEnd = (result) => {
    ...
    if (type === 'COLUMN') {
      ...
      if (source.droppableId === destination.droppableId) {
        ...
        console.log('Updated columns (same list):', newColumns);
        setColumns(newColumns);
      } else {
        ...
        console.log('Updated columns (different lists):', newColumns);
        setColumns(newColumns);
      }
    }
  };
    ...
    const newTaskId =
      Math.max(
        ...Object.values(columns).flatMap((column) =>
          column.tasks.map((task) => parseInt(task.id)),
        ),
      ) + 1;
    const newTask = {
                        ...
                        <h3 {...provided.dragHandleProps}>
                          ...
                          <div className="icon-buttons">
                            <button
                              ...
                              onClick={() => startEditingList(columnId)}
                            >
                              <Edit2 size={16} />
                            </button>
                            <button
